 ...
 * @return Nothing.
 */
export function validateVirtualLocation(
...
export function validateTimeZoneRule(value: TimeZoneRule, path: string): void {
    ...
    if (value.recurrenceRules) {
        value.recurrenceRules.forEach((rule, index) =>
            validateRecurrenceRule(rule, `${path}.recurrenceRules[${index}]`),
        );
    }
    if (value.comments) {
        value.comments.forEach((entry, index) =>
            assertString(entry, `${path}.comments[${index}]`),
        );
    }
}
/**
...
export function validateTimeZoneObject(value: TimeZone, path: string): void {
    ...
    if (value.standard) {
        value.standard.forEach((rule, index) =>
            validateTimeZoneRule(rule, `${path}.standard[${index}]`),
        );
    }
    if (value.daylight) {
        value.daylight.forEach((rule, index) =>
            validateTimeZoneRule(rule, `${path}.daylight[${index}]`),
        );
    }
}
/**
...
export function validateParticipant(value: Participant, path: string): void {
    ...
    if (value.sendTo) {
        if (
            !isObjectValue(value.sendTo) ||
            value.sendTo === null ||
            Array.isArray(value.sendTo)
        ) {
    ...
    if (value.scheduleStatus) {
        value.scheduleStatus.forEach((entry, index) =>
            assertString(entry, `${path}.scheduleStatus[${index}]`),
        );
    }
...
export function validateCommon(value: JSCalendarObject, path: string): void {
    ...
    if (value.excludedRecurrenceRules) {
        value.excludedRecurrenceRules.forEach((rule, index) =>
            validateRecurrenceRule(
                rule,
                `${path}.excludedRecurrenceRules[${index}]`,
            ),
        );
    }
    if (value.recurrenceOverrides) {
        if (
            !isObjectValue(value.recurrenceOverrides) ||
            value.recurrenceOverrides === null ||
    ...
    if (value.virtualLocations) {
        if (
            !isObjectValue(value.virtualLocations) ||
            value.virtualLocations === null ||
            Array.isArray(value.virtualLocations)
        ) {
    ...
    if (value.participants) {
        if (
            !isObjectValue(value.participants) ||
