...
describe("builders", () => {
    it("builds strict objects with @type", () => {
        ...
        const vloc = buildVirtualLocation({
            name: "Zoom",
        ...
        const alert = buildAlert({
            trigger: buildOffsetTrigger({
                offset: JsCal.duration.minutes(-15),
            }),
        });
        const absoluteTrigger = buildAbsoluteTrigger({
    ...
    it("builds id maps with generated ids", () => {
        ...
        const related = JsCal.relatedTo([
            { value: { relation: { parent: true } } },
        ]);
        expect(Object.keys(related).length).toBe(1);
    });

    it("merges id maps with explicit ids", () => {
        const existing = JsCal.locations([
            { id: "loc-1", value: { name: "Room A" } },
        ]);
    ...
    it("rejects wrapper inputs that mix value and direct fields", () => {
        expect(() =>
            JsCal.locations([{ name: "Room A" } as never]),
        ).toThrowError();
    });
    ...
    it("throws when @type mismatches", () => {
        ...
        const bad = { "@type": "Location" } as unknown as Parameters<
            typeof JsCal.Participant
        >[0];
        expect(() => JsCal.Participant(bad)).toThrowError(
            "participant: must have @type Participant",
        );
    });
    ...
    it("throws for mismatched @type across builders", () => {
        // Intentionally bypass type safety to assert @type validation errors.
        expect(() =>
            buildLocation({ "@type": "Participant" } as unknown as Parameters<
                typeof buildLocation
            >[0]),
        ).toThrowError("location: must have @type Location");
        expect(() =>
            buildVirtualLocation({
                "@type": "Location",
            } as unknown as Parameters<typeof buildVirtualLocation>[0]),
        ).toThrowError("virtualLocation: must have @type VirtualLocation");
        expect(() =>
            buildAlert({ "@type": "Link" } as unknown as Parameters<
                typeof buildAlert
            >[0]),
        ).toThrowError("alert: must have @type Alert");
        expect(() =>
            buildOffsetTrigger({ "@type": "Alert" } as unknown as Parameters<
                typeof buildOffsetTrigger
            >[0]),
        ).toThrowError("offsetTrigger: must have @type OffsetTrigger");
        expect(() =>
            buildAbsoluteTrigger({ "@type": "Alert" } as unknown as Parameters<
                typeof buildAbsoluteTrigger
            >[0]),
        ).toThrowError("absoluteTrigger: must have @type AbsoluteTrigger");
        expect(() =>
            buildRelation({ "@type": "Link" } as unknown as Parameters<
                typeof buildRelation
            >[0]),
        ).toThrowError("relation: must have @type Relation");
        expect(() =>
            buildLink({ "@type": "Relation" } as unknown as Parameters<
                typeof buildLink
            >[0]),
        ).toThrowError("link: must have @type Link");
        expect(() =>
            buildTimeZone({ "@type": "Alert" } as unknown as Parameters<
                typeof buildTimeZone
            >[0]),
        ).toThrowError("timeZone: must have @type TimeZone");
        expect(() =>
            buildTimeZoneRule({ "@type": "TimeZone" } as unknown as Parameters<
                typeof buildTimeZoneRule
            >[0]),
        ).toThrowError("timeZoneRule: must have @type TimeZoneRule");
        expect(() =>
            buildRecurrenceRule({ "@type": "NDay" } as unknown as Parameters<
                typeof buildRecurrenceRule
            >[0]),
        ).toThrowError("recurrenceRule: must have @type RecurrenceRule");
        expect(() =>
            buildNDay({ "@type": "RecurrenceRule" } as unknown as Parameters<
                typeof buildNDay
            >[0]),
