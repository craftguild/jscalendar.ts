...
describe("search helpers", () => {
    ...
    it("accepts JsCal instances via JsCal.filterByText", () => {
        const instance = new JsCal.Event({
            start: "2026-02-01T10:00:00",
            title: "Planning",
        });
    ...
    it("computes group range from entries", () => {
        ...
        const results = JsCal.filterByDateRange([group], {
    ...
    it("handles duration ranges with time zones", () => {
        ...
        const results = JsCal.filterByDateRange([tzEvent], {
            start: new Date("2026-02-01T01:30:00Z"),
        });
    ...
    it("converts Date range into event time zone", () => {
        ...
        const results = JsCal.filterByDateRange([eventWithTz], {
    ...
    it("excludes local floating events when Date range is used", () => {
        ...
        const results = JsCal.filterByDateRange([floating], {
            start: new Date("2026-02-01T01:00:00Z"),
        });
        expect(results.length).toBe(0);
    });
    ...
    it("handles Date range with task time zone", () => {
        ...
        const results = JsCal.filterByDateRange([taskWithTz], {
            start: new Date("2026-02-01T01:00:00Z"),
            end: new Date("2026-02-01T01:00:00Z"),
        });
    ...
    it("handles Date range for time zone events and tasks", () => {
        ...
        const results = filterByDateRange([utcEvent.eject(), utcTask.eject()], {
            start: new Date("2026-02-01T01:00:00Z"),
            end: new Date("2026-02-01T01:00:00Z"),
        });
    ...
    it("handles local duration without UTC end calculation", () => {
        ...
        const results = JsCal.filterByDateRange([localEvent], {
            end: "2026-02-01T09:00:00",
        });
        expect(results.length).toBe(0);
    });
    ...
    it("excludes items outside range bounds", () => {
        ...
        const afterRange = filterByDateRange([utcEvent.eject()], {
            start: "2026-02-01T12:30:00",
        });
        expect(afterRange.length).toBe(0);

        const beforeRange = filterByDateRange([utcEvent.eject()], {
    ...
    it("handles Date range for unknown types", () => {
        ...
        // @ts-expect-error unknown object type
        const excluded = filterByDateRange([unknownObject], {
            start: new Date("2026-02-01T00:00:00Z"),
        });
    ...
    it("handles Date range for local tasks as incomparable", () => {
        ...
        const included = JsCal.filterByDateRange(
            [localTask],
            { start: new Date("2026-02-01T00:00:00Z") },
            { includeIncomparable: true },
        );
        expect(included.length).toBe(1);
        const excluded = JsCal.filterByDateRange([localTask], {
            start: new Date("2026-02-01T00:00:00Z"),
        });
        expect(excluded.length).toBe(0);
    });
