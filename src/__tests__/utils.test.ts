...
describe("utils", () => {
    ...
    it("normalizes UTC date-time", () => {
        expect(normalizeUtcDateTime("2026-02-01T00:00:00.000Z")).toBe(
    ...
    it("formats date-time in time zone", () => {
        const value = dateTimeInTimeZone(
    ...
    it("compares UTC date-times", () => {
        expect(
            compareDateTime("2026-02-01T00:00:00Z", "2026-02-01T00:00:00Z"),
        ).toBe(0);
        expect(
            compareDateTime("2026-02-01T00:00:00Z", "2026-02-01T01:00:00Z"),
        ).toBe(-1);
        expect(
            compareDateTime("2026-02-01T01:00:00Z", "2026-02-01T00:00:00Z"),
        ).toBe(1);
    });

    it("returns null for invalid UTC date-times", () => {
        expect(
            compareDateTime("2026-99-99T00:00:00Z", "2026-02-01T00:00:00Z"),
        ).toBeNull();
    });

    it("compares local date-times lexicographically", () => {
        expect(
            compareDateTime("2026-02-01T00:00:00", "2026-02-01T00:00:00"),
        ).toBe(0);
        expect(
            compareDateTime("2026-02-01T00:00:00", "2026-02-02T00:00:00"),
        ).toBe(-1);
    });

    it("returns null when comparing mixed UTC/local", () => {
        expect(
            compareDateTime("2026-02-01T00:00:00", "2026-02-01T00:00:00Z"),
        ).toBeNull();
    });

    it("parses durations", () => {
        expect(durationToMilliseconds("PT1H")).toBe(60 * 60 * 1000);
        expect(durationToMilliseconds("P1DT30M")).toBe(
