 ...
 */
export function findByUid<T extends JSCalendarObject>(
    items: T[],
    uid: string,
): T | undefined {
 ...
 */
export function filterByType<T extends JSCalendarObject>(
    items: T[],
    type: T["@type"],
): T[] {
...
export function filterByDateRange(
    ...
    return items.filter((item) => {
        ...
        const timeZone =
            "timeZone" in item ? (item.timeZone ?? undefined) : undefined;
        const rangeStart = normalizeRangeValue(range.start, timeZone);
        const rangeEnd = normalizeRangeValue(range.end, timeZone);

        if (range.start) {
            const cmpStart = compareDateTime(
                objectRange.end ?? objectRange.start,
                rangeStart,
            );
 ...
 * @return Range or null if unavailable.
 */
function getObjectRangeAsDates(
    item: JSCalendarObject,
...
function getEventRange(event: Event): { start: string; end?: string } {
    ...
    if (isUtcDateTime(event.start)) {
        ...
        if (ms !== null) {
            const end = new Date(Date.parse(event.start) + ms)
                .toISOString()
                .replace(/\.000Z$/, "Z");
 ...
 * @return Group range or null if empty.
 */
function getGroupRangeAsDates(
    group: Group,
): { start: Date; end?: Date } | null {
    const ranges = group.entries
        .map((entry) => getObjectRangeAsDates(entry))
        .filter((range): range is { start: Date; end?: Date } =>
            Boolean(range),
        );
    if (ranges.length === 0) return null;
    const starts = ranges
        ...
        .sort((a, b) => a - b);
    const ends = ranges
        .map((range) => (range.end ?? range.start).getTime())
        .sort((a, b) => a - b);
    const start = starts[0];
    const end = ends[ends.length - 1];
