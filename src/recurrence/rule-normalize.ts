...
export function normalizeRule(
    ...
    if (
        normalized.frequency !== FREQ_SECONDLY &&
        (!normalized.bySecond || normalized.bySecond.length === 0)
    ) {
        normalized.bySecond = [start.second];
    }
    if (
        normalized.frequency !== FREQ_SECONDLY &&
        normalized.frequency !== FREQ_MINUTELY &&
    ...
    if (
        normalized.frequency === FREQ_MONTHLY &&
        (!normalized.byDay || normalized.byDay.length === 0) &&
    ...
    if (
        normalized.frequency === FREQ_YEARLY &&
        (!normalized.byYearDay || normalized.byYearDay.length === 0)
    ) {
        const hasByMonth = normalized.byMonth && normalized.byMonth.length > 0;
        const hasByWeekNo =
            normalized.byWeekNo && normalized.byWeekNo.length > 0;
        const hasByMonthDay =
            normalized.byMonthDay && normalized.byMonthDay.length > 0;
