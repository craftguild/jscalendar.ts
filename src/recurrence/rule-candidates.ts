...
export function generateDateCandidates(
    ...
    const wantsInvalid =
        skip !== SKIP_OMIT && rule.byMonthDay && rule.byMonthDay.length > 0;

    if (rule.frequency === FREQ_YEARLY) {
        for (let month = 1; month <= 12; month += 1) {
            const maxDays = wantsInvalid
                ? 31
                : daysInMonth(periodStart.year, month);
    ...
    if (rule.frequency === FREQ_MONTHLY) {
        const maxDays = wantsInvalid
            ? 31
            : daysInMonth(periodStart.year, periodStart.month);
        for (let day = 1; day <= maxDays; day += 1) {
            const valid =
                day <= daysInMonth(periodStart.year, periodStart.month);
            result.push({
...
export function filterDateCandidates(
    ...
    if (rule.byWeekNo && rule.byWeekNo.length > 0) {
        ...
        result = result.filter(
            (d) => d.valid && matchesByWeekNo(d, byWeekNo, firstDay),
        );
    }
    ...
    if (rule.byYearDay && rule.byYearDay.length > 0) {
        ...
        result = result.filter(
            (d) => d.valid && matchesByYearDay(d, byYearDay),
        );
    }
    ...
    if (byDay && byDay.length > 0) {
        result = result.filter((d) =>
            matchesByDay(d, byDay, rule.frequency, periodStart, firstDay),
        );
    }
